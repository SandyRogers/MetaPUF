...
def generate_clusters(  # noqa: C901
    ...
    study_name: str,
    p_dir: str,
    sample_assembly_info: dict,
):
    """
    generate cluster of assemblies to generate the db
    ...
    if db_size < min_db_size:
        ...
        for cluster in range(1, max_clusters):
            logging.info(
                f"On {cluster = } of {max_clusters = }. Remaining sourmash df len = {len(df1)}"
            )
            if len(df1) < 1:
                break
            else:
        ...
        if assemblies:
            # Some elements are not yet clustered
            cluster_elements[group_no] = remaining_elements
